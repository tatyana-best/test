...
foreach($arParams["BUTTONS"] as $item):
	...
	else
	{
		...
		<a class="<?=$iconClassName !== '' ? htmlspecialcharsbx($iconClassName) : ''?>" href="<?=htmlspecialcharsbx($link)?>" title="<?=htmlspecialcharsbx($title)?>" id="create_deal" <?=$onclick !== '' ? ' onclick="'.htmlspecialcharsbx($onclick).'; return false;"' : ''?><?=$dataAttrs;?>><span class="crm-toolbar-btn-icon"></span><span><?=htmlspecialcharsbx($text)?></span></a><?
		if($alignment !== '')
		{
			?></span><?
		}
